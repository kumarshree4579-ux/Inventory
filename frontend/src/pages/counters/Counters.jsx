...
import { countersAPI, branchesAPI, usersAPI, printersAPI } from '../../api/services';
...
const columns = [
  ...
  { field: 'printer', headerName: 'Printer', flex: 1, renderCell: ({ value }) => value?.name || '—' },
...
const CounterForm = ({ open, onClose, onSaved, editing }) => {
  ...
  useEffect(() => {
    ...
    printersAPI.getAll({ limit: 100, status: 'active' }).then(r => setPrinters(r.data.data || [])).catch(() => {});
    reset(editing
      ? { ...editing, branch: editing.branch?._id || editing.branch || '', cashier: editing.cashier?._id || editing.cashier || '', printer: editing.printer?._id || editing.printer || '' }
      : {}
    );
  }, [open, editing]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!payload.printer) delete payload.printer;
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={12}>
              <FormControl fullWidth size="small">
                ...
                <Controller name="printer" control={control} defaultValue="" render={({ field }) => (
                  <Select {...field} label="Printer">
                    <MenuItem value=""><em>None</em></MenuItem>
                    {printers.map(p => <MenuItem key={p._id} value={p._id}>{p.name}</MenuItem>)}
                  </Select>
                )} />
              </FormControl>
            </Grid>
